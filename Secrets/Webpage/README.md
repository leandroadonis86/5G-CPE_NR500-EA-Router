...
In this router it is possible to make visible hidden debug menus with a builtin account "root" added by manufacturer such as "Engineering Setting" menu. This account unfortually is available in all devices and the password can only be changed by modifying the jbc_settings.ini file with root access in shell.
...
USB device Switch: allow to use router as a modem direct connect to a PC host. It will disable Ethernet if switch on.
